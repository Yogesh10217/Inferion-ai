...
class RecoveryAssuranceEngine:
    ...
    def verify_recovery(self, tenant_id: str, plan_id: str, pre_score: float, post_score: float) -> Dict[str, Any]:
        restored = post_score >= pre_score
        improvement = post_score - pre_score

        logger.info(
            f"Evaluated recovery for plan '{plan_id}' (tenant: '{tenant_id}'): Restored={restored}, Delta={improvement:.4f}"
        )
