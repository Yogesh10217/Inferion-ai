...
class RemediationFailedException(OperationsException):
    def __init__(self, plan_id: str, reason: str) -> None:
        super().__init__(
            message=f"Remediation plan '{plan_id}' failed: {reason}", code="REMEDIATION_FAILED", status_code=422
        )
...
class RunbookExecutionException(OperationsException):
    def __init__(self, runbook_id: str, reason: str) -> None:
        super().__init__(
            message=f"Runbook '{runbook_id}' execution failed: {reason}", code="RUNBOOK_FAILED", status_code=422
        )
