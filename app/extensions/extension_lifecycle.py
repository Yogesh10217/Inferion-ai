...
VALID_EXTENSION_TRANSITIONS = {
    ...
    ExtensionLifecycleState.ENABLED: {
        ExtensionLifecycleState.DISABLED,
        ExtensionLifecycleState.UPDATING,
...
class ExtensionLifecycleManager:
    ...
    def transition(
        self, extension: Extension, target_state: ExtensionLifecycleState, reason: str = "Transition"
        ...
        logger.info(
            f"[EXTENSION LIFECYCLE] '{extension.extension_id}' transitioned {current.value} -> {target_state.value}"
        )
        return target_state
    ...
    def upgrade_extension(self, extension: Extension, new_version: ExtensionVersion) -> ExtensionVersion:
        ...
        self.transition(
            extension,
            ExtensionLifecycleState.UPDATING,
            f"Upgrading from {prev_version_str} to {new_version.version_number}",
        )
        ...
        try:
            ...
            logger.info(
                f"[EXTENSION UPGRADE] Successfully upgraded '{extension.extension_id}' to v{new_version.version_number}"
            )
            return new_version
        except Exception as e:
            logger.error(
                f"[EXTENSION UPGRADE FAILED] Upgrade failed for '{extension.extension_id}': {e}. Initiating rollback..."
            )
    ...
    def rollback_extension(self, extension: Extension, target_version_str: str) -> Extension:
        ...
        if not matched:
            raise RuntimeError(
                f"Version '{target_version_str}' not found in history for extension '{extension.extension_id}'"
            )
