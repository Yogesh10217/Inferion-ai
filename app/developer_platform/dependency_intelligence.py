...
class DependencyManager:
    ...
    def analyze_dependencies(
        self, dependencies: List[Dict[str, str]], tenant_id: str = "global"
    ) -> List[DependencyRisk]:
        risks = []
        for d in dependencies:
            ...
            risk = DependencyRisk(
                dependency_name=name, version=d.get("version", "1.0.0"), risk_level=r_level, tenant_id=tenant_id
            )
            risks.append(risk)

            if r_level in ("HIGH", "CRITICAL"):
                logger.warning(
                    f"[DEPENDENCY INTELLIGENCE] Critical vulnerability detected in dependency '{name}' ({r_level})"
                )
