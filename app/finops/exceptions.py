...
class FinOpsException(AppException):
    ...
    def __init__(
        self, message: str, code: str = "FINOPS_ERROR", status_code: int = 400, details: Optional[Dict[str, Any]] = None
    ) -> None:
...
class OptimizationFailedException(FinOpsException):
    def __init__(self, recommendation_id: str, reason: str) -> None:
        super().__init__(
            message=f"Optimization '{recommendation_id}' failed: {reason}", code="OPTIMIZATION_FAILED", status_code=422
        )
...
class PricingNotFoundException(FinOpsException):
    def __init__(self, provider: str, model_id: str) -> None:
        super().__init__(
            message=f"Pricing for provider '{provider}' and model '{model_id}' not found",
...
class CapacityExceededException(FinOpsException):
    def __init__(self, resource: str, current: float, max_capacity: float) -> None:
        super().__init__(
            message=f"Capacity for '{resource}' exceeded: Current = {current}, Max = {max_capacity}",
