...
class PricingManager:
    ...
    def __init__(self) -> None:
        self._pricing: Dict[str, PricingEntry] = {
            "openai:gpt-4": PricingEntry(
                provider="openai",
                model_id="gpt-4",
                input_token_price_per_1k=Decimal("0.03"),
                output_token_price_per_1k=Decimal("0.06"),
            ),
            "openai:gpt-3.5-turbo": PricingEntry(
                provider="openai",
                model_id="gpt-3.5-turbo",
                input_token_price_per_1k=Decimal("0.0015"),
                output_token_price_per_1k=Decimal("0.002"),
            ),
            "internal:default": PricingEntry(
                provider="internal",
                model_id="default",
                input_token_price_per_1k=Decimal("0.001"),
                output_token_price_per_1k=Decimal("0.001"),
            ),
        }

    def register_pricing(
        self, provider: str, model_id: str, input_price: Decimal, output_price: Decimal, version: str = "1.0.0"
    ) -> PricingEntry:
        ...
        logger.info(
            f"[PRICING MANAGER] Registered pricing for '{key}' (v{version}): Input = ${in_p}/1k, Output = ${out_p}/1k"
        )
        return entry
