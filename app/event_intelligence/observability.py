...
EVENTS_RECEIVED_TOTAL = Counter(
    "ai_events_received_total", "Total enterprise events received", ["category", "tenant_id"]
)
EVENTS_DEDUPLICATED_TOTAL = Counter("ai_events_deduplicated_total", "Total duplicate events suppressed", ["tenant_id"])
EVENTS_CORRELATED_TOTAL = Counter(
    "ai_events_correlated_total", "Total event correlation groups formed", ["correlation_type", "tenant_id"]
)
EVENTS_AUTOMATION_TRIGGERED_TOTAL = Counter(
    "ai_events_automation_triggered_total", "Total automated response plans created", ["action", "tenant_id"]
)
EVENTS_RESOLUTION_DURATION_SECONDS = Histogram(
    "ai_events_resolution_duration_seconds", "Event resolution duration in seconds", ["tenant_id"]
)
...
class EventMetricsCollector:
    ...
    def record_event_correlated(self, correlation_type: str, tenant_id: str) -> None:
        labels = SafeMetricLabelSanitizer.sanitize_labels(
            {"correlation_type": correlation_type, "tenant_id": tenant_id}
        )
