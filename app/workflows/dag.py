...
class DAGBuilder:
    ...
    def add_tool(
        ...
        self.graph.add_node(
            ToolNode(node_id=node_id, name=name, tool_name=tool_name, tool_type=tool_type, config=config)
        )
        return self
    ...
    def add_condition(
        ...
        self.graph.add_node(
            ConditionNode(node_id=node_id, name=name, condition_expression=condition_expression, config=config)
        )
        return self

    def add_parallel(
        self, node_id: str, name: str, branch_nodes: List[str], config: Optional[Dict[str, Any]] = None
    ) -> "DAGBuilder":
    ...
    def connect(
        self, source_id: str, target_id: str, condition: Optional[str] = None, priority: int = 0
    ) -> "DAGBuilder":
