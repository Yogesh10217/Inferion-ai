...
class SecurityIntelligenceManager:
    ...
    def run_full_security_lifecycle_flow(
        ...
        asset = self.asset_manager.register_asset(
            tenant_id, asset_name, SecurityAssetType.MODEL_GATEWAY, SecurityAssetCriticality.CRITICAL
        )
        surf = self.attack_surface_manager.register_attack_surface(
            tenant_id, [AttackSurfaceEntry(asset_id=asset.asset_id, entry_point_name="/v1/chat/completions")]
        )
        ...
        thrt = self.threat_manager.create_threat(
            tenant_id, asset.asset_id, ThreatType.AI_MODEL_THREAT, ThreatSeverity.HIGH
        )
        aithrt = self.ai_threat_manager.analyze_ai_threat(
            tenant_id,
            asset.asset_id,
            AIThreatType.PROMPT_INJECTION,
            severity=AIThreatSeverity.HIGH,
            signal_id=sig.signal_id,
        )
        self.metrics_collector.record_threat(thrt.threat_type.value, tenant_id)

        # 4. Discover Vulnerability
        vuln = self.vulnerability_manager.create_vulnerability(
            tenant_id,
            asset.asset_id,
            "Unsanitized Prompt Context Vulnerability",
            VulnerabilitySeverity.HIGH,
            cve_id="CVE-2026-AI01",
        )
        self.vulnerability_manager.transition_vulnerability(
            vuln.vulnerability_id, tenant_id, VulnerabilityStatus.VALIDATING
        )
        self.vulnerability_manager.transition_vulnerability(
            vuln.vulnerability_id, tenant_id, VulnerabilityStatus.CONFIRMED
        )

        # 5. Analyze Attack Path & Correlate
        path = self.attack_path_analyzer.analyze_attack_path(
            tenant_id, entry_node_name=asset_name, target_asset_name="User_Context_DB"
        )
        ...
        inc = self.incident_manager.create_incident(
            tenant_id, asset.asset_id, f"Prompt Injection Attack on {asset_name}", SecurityIncidentSeverity.SEV_1_HIGH
        )
        ...
        ev = SecurityEvidence(
            tenant_id=tenant_id, source="API_GATEWAY", content_reference=sig.signal_id, metadata={"sanitized": True}
        )
        ...
        inv = self.investigation_manager.create_investigation(
            tenant_id, inc.incident_id, f"Investigation for {inc.incident_id}"
        )
        concluded_inv = self.investigation_manager.conclude_investigation(
            inv.investigation_id, tenant_id, [InvestigationFinding(description="Confirmed malicious prompt injection")]
        )
        ...
        self.incident_manager.transition_incident(
            inc.incident_id, tenant_id, SecurityIncidentStatus.CONTAINMENT_PLANNED
        )
        self.incident_manager.transition_incident(
            inc.incident_id, tenant_id, SecurityIncidentStatus.REMEDIATION_PENDING
        )
        self.incident_manager.transition_incident(
            inc.incident_id, tenant_id, SecurityIncidentStatus.REMEDIATION_IN_PROGRESS
        )
        ...
        closed_inc = self.incident_manager.transition_incident(
            inc.incident_id, tenant_id, SecurityIncidentStatus.CLOSED
        )
        ...
        learning = self.learning_manager.record_learning(
            tenant_id, inc.incident_id, "Prompt Injection Pattern", "Strict Context Boundary Policy"
        )
