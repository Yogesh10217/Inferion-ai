...
if PROMETHEUS_AVAILABLE:
    GOVERNANCE_EVALUATIONS_TOTAL = Counter(
        "ai_governance_policy_evaluations_total", "Total policy evaluations", ["tenant_id", "decision"]
    )
    GOVERNANCE_VIOLATIONS_TOTAL = Counter(
        "ai_governance_policy_violations_total", "Total governance violations", ["tenant_id", "severity"]
    )
    GOVERNANCE_RISK_SCORE = Gauge("ai_governance_risk_score", "Average risk score", ["tenant_id"])
    GOVERNANCE_COMPLIANCE_SCORE = Gauge(
        "ai_governance_compliance_score", "Compliance score percentage", ["tenant_id", "framework"]
    )
    GOVERNANCE_APPROVAL_REQUIRED = Counter(
        "ai_governance_approval_required_total", "Total actions requiring approval", ["tenant_id"]
    )
    GOVERNANCE_REMEDIATIONS_TOTAL = Counter(
        "ai_governance_remediations_total", "Total control enforcement remediations", ["tenant_id", "action"]
    )
    GOVERNANCE_EVIDENCE_TOTAL = Counter(
        "ai_governance_evidence_collected_total", "Total evidence records collected", ["tenant_id", "source"]
    )
