...
class DataFabricException(AppException):
    ...
    def __init__(self, message: str, code: str = "DATA_FABRIC_ERROR", status_code: int = 400, details: Optional[Dict[str, Any]] = None) -> None:
...
class ConnectorNotFoundException(DataFabricException):
    def __init__(self, connector_type: str) -> None:
        super().__init__(message=f"Connector for type '{connector_type}' not found or unregistered", code="CONNECTOR_NOT_FOUND", status_code=404)


class ConnectorAuthenticationError(DataFabricException):
    def __init__(self, connector_type: str, reason: str) -> None:
        super().__init__(message=f"Authentication failed for connector '{connector_type}': {reason}", code="CONNECTOR_AUTH_ERROR", status_code=401)
...
class DataIngestionError(DataFabricException):
    def __init__(self, source_id: str, reason: str) -> None:
        super().__init__(message=f"Ingestion failed for data source '{source_id}': {reason}", code="DATA_INGESTION_ERROR", status_code=500)


class DataSynchronizationError(DataFabricException):
    def __init__(self, sync_job_id: str, reason: str) -> None:
        super().__init__(message=f"Synchronization job '{sync_job_id}' failed: {reason}", code="DATA_SYNC_ERROR", status_code=500)


class SchemaDiscoveryError(DataFabricException):
    def __init__(self, source_id: str, reason: str) -> None:
        super().__init__(message=f"Schema discovery failed for data source '{source_id}': {reason}", code="SCHEMA_DISCOVERY_ERROR", status_code=500)
...
class ChangeDataCaptureError(DataFabricException):
    def __init__(self, source_id: str, reason: str) -> None:
        super().__init__(message=f"CDC processing failed for source '{source_id}': {reason}", code="CDC_ERROR", status_code=500)
