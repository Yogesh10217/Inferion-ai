...
class SensitiveDataDetector:
    ...
    PATTERNS = {
        "API_KEY": re.compile(
            r"(?:api[_-]?key|sk[_-]live|secret[_-]?key)\s*[:=]\s*['\"]?([A-Za-z0-9_\-]{16,})['\"]?", re.IGNORECASE
        ),
    ...
    def detect_sensitive_data(self, text_or_dict: Any) -> List[DetectedSensitivity]:
        ...
        for name, pattern in self.PATTERNS.items():
            ...
            if matches:
                ...
                detections.append(
                    DetectedSensitivity(pattern_name=name, match_count=len(matches), sample_masked=masked_sample)
                )
    ...
    def sanitize(self, content: Any, action: SensitiveDataAction = SensitiveDataAction.MASK) -> Any:
        ...
        if action == SensitiveDataAction.REDACT:
            ...
            for name, pattern in self.detector.PATTERNS.items():
                text = pattern.sub(
                    lambda m: (
                        m.group(0)[:2] + "*" * max(0, len(m.group(0)) - 4) + m.group(0)[-2:]
                        if len(m.group(0)) > 4
                        else "****"
                    ),
                    text,
