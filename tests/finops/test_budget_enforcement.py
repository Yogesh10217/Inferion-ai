...
from app.finops.budgets import BudgetAction, BudgetManager
...
def test_budget_enforcement_actions():
    ...
    # 1. BLOCK policy
    b_block = mgr.create_budget(
        "Block Budget", Decimal("50.0"), tenant_id="t_block", enforcement_action=BudgetAction.BLOCK
    )
    ...
    # 3. THROTTLE policy
    b_throt = mgr.create_budget(
        "Throttle Budget", Decimal("50.0"), tenant_id="t_throt", enforcement_action=BudgetAction.THROTTLE
    )
