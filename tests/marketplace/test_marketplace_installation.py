...
import pytest
from app.marketplace.marketplace_item import MarketplaceItem, MarketplaceCategory
...
def test_marketplace_item_installation():
    ...
    m = ExtensionManifest(identifier="ext.install", name="Install Ext", publisher_id="pub_1", extension_type=ExtensionType.TOOL)
    item = MarketplaceItem(title="Installable Extension", category=MarketplaceCategory.TOOLS, publisher_id="pub_1", manifest=m)
