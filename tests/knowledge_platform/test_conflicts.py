...
from app.knowledge_platform.conflicts import KnowledgeConflictManager, ConflictType, ConflictResolutionStrategy


def test_conflict_detection_and_resolution():
    km = KnowledgeManager()
    item_a = km.create_knowledge_item("Report 2026 - Source A", content="Revenue 10M", tenant_id="t_cnf", confidence_score=0.9)
    item_b = km.create_knowledge_item("Report 2026 - Source B", content="Revenue 12M", tenant_id="t_cnf", confidence_score=0.7)

    cm = KnowledgeConflictManager(knowledge_manager=km)
    cnflct = cm.detect_conflict(item_a.item_id, item_b.item_id, conflict_type=ConflictType.FACT_CONTRADICTION, tenant_id="t_cnf")
