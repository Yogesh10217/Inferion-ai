...
from app.knowledge_platform.conflicts import ConflictResolutionStrategy, ConflictType
...
from app.knowledge_platform.knowledge import KnowledgeStatus
...
def test_flow_1_governed_enterprise_retrieval():
    ...
    item1 = mgr.create_and_index_knowledge(
        "Standard Operating Procedure", content="Public SOP", classification="INTERNAL", tenant_id="t_kf1"
    )
    mgr.create_and_index_knowledge(
        "Secret Financial Model", content="Restricted revenue forecasts", classification="RESTRICTED", tenant_id="t_kf1"
    )
...
def test_flow_3_stale_knowledge_detection():
    ...
    stale_ids = mgr.freshness_evaluator.process_cdc_event(
        "src_pg_100", tenant_id="t_kf3", reason=StalenessReason.SOURCE_CHANGED
    )
...
def test_flow_4_knowledge_conflict():
    ...
    # Conflict detected
    cnflct = mgr.conflict_manager.detect_conflict(
        item_a.item_id, item_b.item_id, conflict_type=ConflictType.FACT_CONTRADICTION, tenant_id="t_kf4"
    )
...
def test_flow_5_context_budget_optimization():
    ...
    mgr.create_and_index_knowledge("Large Text Doc", content="Word " * 500, tenant_id="t_kf5")
    ...
    cmp_res = mgr.context_compressor.compress_context(
        cwin, strategy=CompressionStrategy.DEDUPLICATION, target_ratio=0.5
    )
...
def test_flow_6_multi_hop_knowledge_graph_retrieval():
    ...
    mgr.knowledge_graph_manager.add_edge(
        n1.node_id, n2.node_id, relationship=KnowledgeRelationship.USES, tenant_id="t_kf6"
    )
    mgr.knowledge_graph_manager.add_edge(
        n2.node_id, n3.node_id, relationship=KnowledgeRelationship.OWNED_BY, tenant_id="t_kf6"
    )
...
def test_flow_7_agent_plus_workflow_organizational_learning():
    ...
    insight_id = mgr.workflow_adapter.record_workflow_insight(
        "wf_prod_run_1",
        title="Deployment Insight",
        content="Postgres connection pool size 20 optimal",
        tenant_id="t_kf7",
    )
    assert insight_id.startswith("kitem_")

    # Future agent retrieves this insight
    ret_res = mgr.agent_adapter.query_knowledge_for_agent(
        "agent_optimizer", query="Postgres connection pool", tenant_id="t_kf7"
    )
...
def test_flow_8_strict_cross_tenant_isolation():
    ...
    # Tenant A retrieve
    res_A = mgr.retrieval_pipeline.execute_retrieval(
        RetrievalRequest(query="Secret", tenant_id="Tenant_A", identity_id="user_a")
    )
    assert len(res_A.items) == 1
    assert res_A.items[0]["title"] == "Tenant A Doc"

    # Tenant B retrieve
    res_B = mgr.retrieval_pipeline.execute_retrieval(
        RetrievalRequest(query="Secret", tenant_id="Tenant_B", identity_id="user_b")
    )
