...
from app.identity.authentication import AuthenticationAssuranceLevel, AuthenticationMethod
from app.identity.credentials import CredentialStatus, CredentialType
...
from app.identity.privileged_access import PrivilegedAccessStatus, PrivilegedRole
from app.identity.risk import AnomalyType, IdentityRiskSeverity
from app.identity.zero_trust import ZeroTrustAction
...
def test_e2e_flow_1_high_risk_privileged_access():
    ...
    ident = mgr.identity_manager.create_identity(
        "admin_db", identity_type=IdentityType.ADMINISTRATOR, tenant_id="t_flow1"
    )

    # 2. Authentication assurance evaluated
    auth_res = mgr.auth_manager.authenticate(
        ident.identity_id, method=AuthenticationMethod.MFA, mfa_verified=True, tenant_id="t_flow1"
    )
    ...
    grant = mgr.privileged_access_manager.request_privileged_access(
        ident.identity_id, PrivilegedRole.DATABASE_ADMIN, tenant_id="t_flow1", duration_minutes=15
    )
    ...
    aud_evt = mgr.audit_manager.record_event(
        "PRIVILEGED_ACCESS", ident.identity_id, "db_access", grant.grant_id, "GRANTED", tenant_id="t_flow1"
    )
...
def test_e2e_flow_2_suspicious_session():
    ...
    zt_res = mgr.zero_trust_engine.evaluate(
        ident.identity_id, tenant_id="t_flow2", network_trusted=False, risk_score=50.0
    )
...
def test_e2e_flow_3_agent_delegated_authorization():
    ...
    assert (
        mgr.agent_identity_manager.validate_agent_action(
            del_auth.delegation_id, "search_docs", requested_scope="search"
        )
        is True
    )
    ...
    with pytest.raises(AgentBoundaryViolationException):
        mgr.agent_identity_manager.validate_agent_action(
            del_auth.delegation_id, "export_full_database", requested_scope="export"
        )
...
def test_e2e_flow_4_credential_rotation():
    ...
    cred = mgr.credential_manager.create_credential(
        "Payment Gateway API Key",
        identity_id="service_pay",
    ...
    # Record Audit
    aud = mgr.audit_manager.record_event(
        "TOKEN_ROTATION", "service_pay", "rotate_key", new_cred.credential_id, "ROTATED", tenant_id="t_flow4"
    )
...
def test_e2e_flow_7_tenant_isolation():
    ...
    mgr.credential_manager.create_credential("cred_A", identity_id=id_A.identity_id, tenant_id="Tenant_A")

    # Tenant B Setup
    id_B = mgr.identity_manager.create_identity("user_B", tenant_id="Tenant_B")
    mgr.session_manager.create_session(id_B.identity_id, tenant_id="Tenant_B")
    mgr.credential_manager.create_credential("cred_B", identity_id=id_B.identity_id, tenant_id="Tenant_B")
