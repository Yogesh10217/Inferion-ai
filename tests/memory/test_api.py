...
async def test_create_and_get_memory_api(get_client, admin_token_headers):
    async with get_client() as client:
        resp = await client.post("/v1/memory", json={"content": "Test memory content from API"}, headers=admin_token_headers)
...
async def test_list_and_search_memory_api(get_client, admin_token_headers):
    async with get_client() as client:
        ...
        search_resp = await client.post("/v1/memory/search", json={"query": "Searchable", "top_k": 5}, headers=admin_token_headers)
...
async def test_profile_memory_api(get_client, admin_token_headers):
    async with get_client() as client:
        ...
        patch_resp = await client.patch("/v1/memory/profile?user_id=test_user", json={"profile_data": {"preferred_language": "go"}}, headers=admin_token_headers)
