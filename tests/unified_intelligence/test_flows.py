...
from app.unified_intelligence.situation_awareness import SituationStatus, SituationSeverity
...
def test_flow_01_domain_input_normalization_and_ingestion(manager):
    ...
    assert sig.signal_id.startswith("norm-sig-") or sig.signal_id.startswith("unif-sig-") or sig.signal_id.startswith("sig-")
...
def test_flow_04_cross_domain_signal_correlation_and_confidence(manager):
    ...
    manager.ingest_domain_input("tenant_a", UnifiedDomainInput(
        domain=IntelligenceDomain.SECURITY,
        tenant_id="tenant_a",
...
def test_flow_05_explainable_causal_hypothesis_generation(manager):
    ...
    if not corr_group:
        ...
        manager.ingest_domain_input("tenant_a", UnifiedDomainInput(
            domain=IntelligenceDomain.SECURITY, tenant_id="tenant_a", entity_reference="sys-1", signal_type="EXPLOIT"
        ))
        corr_group = manager.correlation_engine.correlate_context(manager.fuse_context("tenant_a"))
...
def test_flow_06_situation_awareness_lifecycle_and_severity_separation(manager):
    ...
    manager.ingest_domain_input("tenant_a", UnifiedDomainInput(
        domain=IntelligenceDomain.SECURITY, tenant_id="tenant_a", entity_reference="node-1", signal_type="BREACH", severity="CRITICAL"
    ))
    manager.ingest_domain_input("tenant_a", UnifiedDomainInput(
        domain=IntelligenceDomain.OPERATIONS, tenant_id="tenant_a", entity_reference="node-1", signal_type="OUTAGE", severity="CRITICAL"
    ))
...
def test_flow_07_cross_domain_risk_propagation_graph(manager):
    ...
    graph = manager.risk_propagation_engine.build_graph("tenant_a", [
    ...
    propagated = manager.risk_propagation_engine.propagate_risk("tenant_a", graph, IntelligenceDomain.IDENTITY, initial_risk=0.9)
...
def test_flow_08_multi_domain_unified_impact_analysis(manager):
    ...
    manager.ingest_domain_input("tenant_a", UnifiedDomainInput(
        domain=IntelligenceDomain.SECURITY, tenant_id="tenant_a", entity_reference="db-prod", signal_type="DATA_EXFILTRATION", severity="HIGH"
    ))
    situations = manager.detect_situations("tenant_a")
    assert len(situations) >= 1
...
def test_flow_09_holistic_enterprise_risk_engine_evaluation(manager):
    ...
    manager.ingest_domain_input("tenant_a", UnifiedDomainInput(
        domain=IntelligenceDomain.SECURITY, tenant_id="tenant_a", entity_reference="svc-auth", signal_type="ATTACK", risk_score=0.85
    ))
...
def test_flow_12_prioritized_recommendation_generation(manager):
    ...
    manager.ingest_domain_input("tenant_a", UnifiedDomainInput(
        domain=IntelligenceDomain.SECURITY, tenant_id="tenant_a", entity_reference="res-1", signal_type="FAIL", severity="HIGH"
    ))
...
def test_flow_14_human_governance_approval_gate_enforcement(manager):
    ...
    manager.ingest_domain_input("tenant_a", UnifiedDomainInput(
        domain=IntelligenceDomain.SECURITY, tenant_id="tenant_a", entity_reference="res-1", signal_type="FAIL", severity="CRITICAL"
    ))
    sits = manager.detect_situations("tenant_a")
    recs = manager.generate_recommendations("tenant_a", sits[0].situation_id)
    rec = recs[0]
...
def test_flow_16_autonomous_delegation_request_construction(manager):
    ...
    manager.ingest_domain_input("tenant_a", UnifiedDomainInput(
        domain=IntelligenceDomain.SECURITY, tenant_id="tenant_a", entity_reference="res-1", signal_type="FAIL", severity="LOW"
    ))
    sits = manager.detect_situations("tenant_a")
    recs = manager.generate_recommendations("tenant_a", sits[0].situation_id)
    plan = manager.build_coordination_plan("tenant_a", recs[0].recommendation_id)
...
def test_flow_17_delegation_verification_and_outcome_tracking(manager):
    ...
    res = manager.verification_engine.verify_delegation("tenant_a", delegation, {"success": True, "message": "Quarantined model."})
...
def test_flow_18_sha256_immutable_evidence_ledger_sealing(manager):
    ...
    record = manager.evidence_ledger.create_evidence("tenant_a", "security", {"action": "quarantine", "entity": "node-1"})
...
def test_correlation_does_not_claim_causation(manager):
    ...
    manager.ingest_domain_input("tenant_causal", UnifiedDomainInput(
        domain=IntelligenceDomain.SECURITY, tenant_id="tenant_causal", entity_reference="sys-x", signal_type="LOG_EVENT"
    ))
