...
from app.governance_platform.risk import RiskCategory, RiskFactor, RiskSeverity, RiskStatus
from app.governance_platform.policy_evaluation import GovernanceDecision
from app.governance_platform.compliance import FrameworkType, ComplianceControl, ComplianceStatus
...
from app.governance_platform.violations import ViolationType, ViolationSeverity, ViolationStatus
...
def test_flow_1_high_risk_agent_action_pipeline():
    ...
    pol_res = mgr.policy_evaluator.evaluate_request("access", "dataset_restricted_financial", tenant_id="f1", actor_id="agent_1", context={"is_data_access": True})
    ...
    risk_ass = mgr.risk_manager.calculate_risk("dataset_restricted_financial", factors, category=RiskCategory.DATA, tenant_id="f1")
    assert risk_ass.severity == RiskSeverity.HIGH

    # 3. Human oversight requires approval for high risk
    ovs_pol = mgr.human_oversight_engine.create_policy("Agent Data Access Policy", "dataset_restricted_financial", AutonomyLevel.CONSTRAINED_AUTONOMOUS, OversightLevel.APPROVAL_REQUIRED, tenant_id="f1")
    ...
    # 4. Record evidence
    evd = mgr.evidence_collector.collect_evidence(EvidenceSource.POLICY_DECISION, pol_res.evaluation_id, "dataset_restricted_financial", pol_res.model_dump(), tenant_id="f1")
...
def test_flow_2_model_safety_regression_pipeline():
    ...
    scan = mgr.monitoring_engine.run_monitoring_scan(tenant_id="f2", target_resource_id="model_gpt4_v2")
    ...
    rem = mgr.enforcement_engine.plan_remediation("model_gpt4_v2", EnforcementAction.BLOCK_DEPLOYMENT, risk_level=RemediationRisk.LOW, tenant_id="f2")
...
def test_flow_3_critical_data_violation_containment():
    ...
    rem = mgr.enforcement_engine.plan_remediation("dataset_customer_pii", EnforcementAction.REVOKE_ACCESS, risk_level=RemediationRisk.CRITICAL, is_emergency=True, tenant_id="f3")
    ...
    evd = mgr.evidence_collector.collect_evidence(EvidenceSource.INCIDENT, v.incident_id or "inc_1", "dataset_customer_pii", v.model_dump(), tenant_id="f3")
    assert evd.content_hash != ""
...
def test_flow_4_compliance_evidence_assessment():
    ...
    e1 = mgr.evidence_collector.collect_evidence(EvidenceSource.AUDIT_LOG, "rec_1", "res_1", {"action": "LOGIN"}, tenant_id="f4")
    e2 = mgr.evidence_collector.collect_evidence(EvidenceSource.DEPLOYMENT, "dep_1", "res_1", {"action": "DEPLOY"}, tenant_id="f4")

    # 2. Controls mapped & framework assessed
    ctrls = [
        ComplianceControl(control_id="CC1.1", name="Audit Evidence", status=ComplianceStatus.COMPLIANT, evidence_ids=[e1.evidence_id]),
        ComplianceControl(control_id="CC2.1", name="Deployment Gating", status=ComplianceStatus.COMPLIANT, evidence_ids=[e2.evidence_id]),
    ]
...
def test_flow_5_autonomous_agent_boundary():
    ...
    pol = mgr.human_oversight_engine.create_policy("Constrained Agent Policy", "agent_worker_x", AutonomyLevel.CONSTRAINED_AUTONOMOUS, config_modification_allowed=False, tenant_id="f5")

    # 2. Agent attempts privileged config modification
    res = mgr.human_oversight_engine.evaluate_action_autonomy(pol.policy_id, "modify_prod_config", is_config_change=True)

    assert res["allowed"] is False
    assert res["requires_approval"] is True

    # 3. Violation recorded & explanation generated
    v = mgr.violation_manager.record_violation("Autonomy Boundary Breach Attempt", ViolationType.AUTONOMY_VIOLATION, ViolationSeverity.MEDIUM, "agent_worker_x", tenant_id="f5")
...
def test_flow_6_high_risk_remediation():
    ...
    rem = mgr.enforcement_engine.plan_remediation("deploy_prod_v3", EnforcementAction.ROLLBACK_DEPLOYMENT, risk_level=RemediationRisk.HIGH, tenant_id="f6")
...
def test_flow_7_tenant_isolation_verification():
    ...
    mgr.evidence_collector.collect_evidence(EvidenceSource.AUDIT_LOG, "rec_A", "res_A", {"data": "A"}, tenant_id="Tenant_A")
    mgr.violation_manager.record_violation("Viol A", ViolationType.POLICY_VIOLATION, ViolationSeverity.LOW, "res_A", tenant_id="Tenant_A")
    ...
    mgr.evidence_collector.collect_evidence(EvidenceSource.AUDIT_LOG, "rec_B", "res_B", {"data": "B"}, tenant_id="Tenant_B")
    mgr.violation_manager.record_violation("Viol B", ViolationType.POLICY_VIOLATION, ViolationSeverity.LOW, "res_B", tenant_id="Tenant_B")
