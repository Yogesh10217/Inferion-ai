...
def test_evidence_hashing_and_chaining():
    ...
    e1 = collector.collect_evidence(
        EvidenceSource.DEPLOYMENT, "dep_1", "res_1", {"status": "SUCCESS"}, tenant_id="t_hash"
    )
    e2 = collector.collect_evidence(
        EvidenceSource.APPROVAL, "appr_1", "res_1", {"status": "APPROVED"}, tenant_id="t_hash"
    )
