...
async def test_intelligence_api_signal_ingest(get_client, admin_token_headers):
    async with get_client() as client:
        res = await client.post(
            ...
            json={"source": "OPERATIONS", "signal_type": "METRIC_THRESHOLD", "message": "High CPU utilization", "tenant_id": "tenant_api"},
...
async def test_intelligence_api_forecast(get_client, admin_token_headers):
    async with get_client() as client:
        res = await client.post(
            ...
            json={"target_resource_id": "res_svc_api", "forecast_type": "INCIDENT_RISK_FORECAST", "tenant_id": "tenant_api"},
            headers=admin_token_headers,
        )
