...
import pytest
from app.intelligence_platform.recommendations import RecommendationManager, RecommendationType, RecommendationStatus
...
def test_recommendation_idempotency_and_superseding():
    ...
    rec1 = mgr.create_recommendation("t1", RecommendationType.ROLLBACK_DEPLOYMENT, "Rollback Deploy", "Rollback action", "svc_1", "Risk reduction", idempotency_key="key_123")

    # Idempotent call returns original recommendation
    rec1_dup = mgr.create_recommendation("t1", RecommendationType.ROLLBACK_DEPLOYMENT, "Rollback Deploy", "Rollback action", "svc_1", "Risk reduction", idempotency_key="key_123")
    assert rec1_dup.recommendation_id == rec1.recommendation_id

    # Create new recommendation and supersede old one
    rec2 = mgr.create_recommendation("t1", RecommendationType.SCALE_RESOURCE, "Scale Deploy", "Scale action", "svc_1", "Performance boost", idempotency_key="key_456")
