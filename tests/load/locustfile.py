...
class HighConcurrencyInferenceUser(FastHttpUser):
    ...
    def test_non_streaming_chat_completion(self):
        ...
        with self.client.post(
            "/v1/chat/completions", json=payload, headers=self.headers, catch_response=True
        ) as response:
    ...
    def test_streaming_chat_completion(self):
        payload = {
            "model": random.choice(MODELS),
            "messages": [{"role": "user", "content": "Stream response chunk validation for 10k RPS load test."}],
            "stream": True,
            "metadata": {"mock": True, "benchmark": "10k_rps"},
        }
        with self.client.post(
    ...
    def test_knowledge_rag_search(self):
        payload = {"query": "high concurrency 10k RPS benchmark performance", "top_k": 5, "vector_store": "FAISS"}
        with self.client.post(
            "/v1/knowledge/search", json=payload, headers=self.headers, catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
    ...
    def test_agent_execution(self):
        payload = {"agent_id": "agt-dev-assistant", "prompt": "Run automated test verification", "strategy": "ReAct"}
        with self.client.post(
            "/v1/agents/execute", json=payload, headers=self.headers, catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
